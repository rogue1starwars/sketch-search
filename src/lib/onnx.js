...
async function predict(image_path = "./image.png", resize = true) {
  try {
    const modelPath = path.join(__dirname, "model.onnx");
    const session = await ort.InferenceSession.create(modelPath).catch(
      console.error,
    );
    const [input_tensor, img_width, img_height] =
      await get_image_tensor_from_path(image_path);
    const feeds = {};
